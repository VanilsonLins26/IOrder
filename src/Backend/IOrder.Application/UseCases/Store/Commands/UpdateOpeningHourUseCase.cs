...
public class UpdateOpeningHourUseCase : IUpdateOpeningHourUseCase
{
    ...
    private readonly IValidator<UpdateOpeningHourRequestDto> _validator;

    public UpdateOpeningHourUseCase(IStoreWriteOnlyRepository writeOnlyRepository, IUnitOfWork uof, ILoggedUserService loggedUserService, IStorePermissionService storePermissionService, IValidator<UpdateOpeningHourRequestDto> validator)
    {
    ...
    public async Task<StoreResponseDto> Execute(UpdateOpeningHourRequestDto request, Guid storeId)
    {
        ...
        var newHours = request.OpeningHours.Select(req => new OpeningHour
        {
